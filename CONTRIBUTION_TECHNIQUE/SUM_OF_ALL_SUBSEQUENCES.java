...
public class SUM_OF_ALL_SUBSEQUENCES {
    final static long mod = 1_000_000_007L;
    static PrintWriter out;

    /*
    Q. Find the sum of all subsequences of the array of length N.

    Brute force --> Build all 2^n subsequences and then sum 
    Time Complexity -->  O(2^n + n ) 

    Optimal Approach -->
    rather than counting all the subsequences count in how
    many subsequences my index i is contributing

    it is 2^(n-1) contributions so the contribution to the sum is
     for each element the contribution to the sum is -- > arr[i] * 2^(n-1);
    */
    ...
    static void solve( int n , int [] arr ) {
        // your solution here
    	// O(n) each element i contributes to 2^(n-1) times to the sum
    	...
    	// Optimal --> take the sum during input itself and then just
    	// return sum * (1 << (n-1))
