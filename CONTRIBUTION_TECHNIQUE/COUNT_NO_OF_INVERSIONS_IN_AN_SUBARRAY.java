...
public class COUNT_NO_OF_INVERSIONS_IN_AN_SUBARRAY {
    ...
    Q. Find the number of inversions in all subarrays of the array of length N.
     	Inversion is count of pairs (i , j) such that i < j and and arr[i] > arr[j]
		
		n <= 1e3 --(n^2 possible)


		Brute force --> 
		Generate all the subarrays then in each subarray generate all the pairs (i , j)
		This will take O(n*n * (n*n)) == O(n^4)


     	Hint -- Counting all the subarrays will be  O(n^2) -- not possible 
     	thinking direction --> hmmm subarrays could be contribution....

     	Now we have two moving parts i and j ... so fix one of them 
     	lets fix j 
     	So for each index i < j how many pairs --- no no 

     	let's say we found a pair i , j then how in how many subarrays will it come ?? 

     	[0........i......j........n]   

     	contribution see by drawing (i+1) * (n-j) == for each (pair i , j)

     	//https://www.geeksforgeeks.org/dsa/counting-inversions-in-an-subarrays/
