...
public class intuit_goblet_of_fire implements Runnable {
    final static long mod = 1_000_000_007L;
    static PrintWriter out;


    public static void main(String[] args) {
        new Thread(null, new intuit_goblet_of_fire (), "whatever", 1 << 30).start();
    }
    ...
    static void solve(FastReader sc ) throws Exception {
        ...
        while(t-- > 0) {
        	...
        	int count = 0;
        	for( int i = 0 ; i < n ; i++) {
        		int offset = bs(p[i] +1 , arr);
        		if(offset >= n) {
        			count++;
        		}
        		else {
        			if(s[i] > suffix_max[offset]) {
        				count++;
        			}
        		}
        	}
