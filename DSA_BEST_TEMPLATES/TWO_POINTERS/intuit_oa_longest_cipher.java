...
public class intuit_oa_longest_cipher implements Runnable {
    final static long mod = 1_000_000_007L;
    static PrintWriter out;


    public static void main(String[] args) {
        new Thread(null, new intuit_oa_longest_cipher (), "whatever", 1 << 30).start();
    }
    ...
    static void solve(FastReader sc ) throws Exception {
        ...
        while(t-- > 0) {
        	...
        	int n = s.length();
           	...
           	int ans = -1;
           	while(tail < n) {
           		while(head + 1 < n  && check(head+1 , digits , s)) {
           			head++;
           			if(Character.isUpperCase(s.charAt(head))) {
           				uppercase++;
           			}
           		}
           		if(digits == 0 && uppercase >=1 ) {
           		...
           		if(tail > head) {
           			...
           			if(Character.isUpperCase(s.charAt(tail))){
           				uppercase--;
           			}
           			tail++;
           		}
    ...
    static boolean check(int head , int digits , String s) {
    	if(digits==0 && !Character.isDigit(s.charAt(head))) return true;
    	else return false;
    }
