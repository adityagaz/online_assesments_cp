...
public class hackerrank_oa_java_hashing {
    ...
    static void solve(int n , int k , String [] str ) {
        
        int [] arr = new int[1442];

        int startDay = parseString("00:00");
        int endDay = parseString("23:59");
        for( int i  = 0 ; i < n ; i++) {
        	String s = str[i];
        	int startMinute = parseString(s.split(" ")[2]);
        	int endMinute = parseString(s.split(" ")[3]);	
        	arr[startMinute] += 1;
        	if (endMinute < endDay) arr[endMinute+1] -=1;
        }
        ...
        for( int i = 0; i <= 1440 ; i++) {
        	...
        	if(isTime) {
        		System.out.println(reverseParse(i));
        		return;
        	}
        	}
        }
    ...
    static String reverseParse(int min ) {
    	int minutes = min %60;
    	int hours = min/60; //floor division
    ...
    static int parseString(String str) {
    	int hr = Integer.parseInt(str.substring(0, 2))*60;
