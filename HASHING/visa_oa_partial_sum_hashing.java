...
public class visa_oa_partial_sum_hashing {
    ...
    public static void main(String[] args) throws Exception {
        ...
        while(t-- > 0) {
    		...
    		int mx = -1;
        	for( int i  = 0 ; i < p ; i++ ) queries[i] = sc.nextInt();
            ...
            // solve(lamps , queries , n ,p , mx );
            solveSweepLine(lamps , queries , n ,p , mx );
            System.out.println();
        }
    ...
    static void solveSweepLine( int [][] lamps , int [] queries , int n , int p , int max ) {
        

        //easier way to do using Pair class...
        // build the class and then sort it and take prefix sum
        Map<Integer, Long> mp = new TreeMap<>();

        for( int i = 0 ; i < n ; i++ ) {
        	int l = lamps[i][0];
        	int r = lamps[i][1];
        	mp.put(l , mp.getOrDefault(l , 0L)+1L);
        	mp.put(r+1 , mp.getOrDefault(r+1, 0L)-1L);	
        }

        for( int q : queries) {
        	mp.put(q, mp.getOrDefault(q , 0L));
        }
        long sum = 0L;
    ...
    static void solve( int [][] lamps , int [] queries , int n , int p , int max ) {
        
    	//assuming the lamps[i][0] >=1 and queries[i] >= 1 as constraints
