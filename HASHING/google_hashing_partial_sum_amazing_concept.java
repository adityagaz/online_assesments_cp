...
public class google_hashing_partial_sum_amazing_concept {
...
*/
    static void solve(int n , int [] arr , int k  ) {
		// no need to take care of array indices....
       	//sweepline
       	TreeMap<Integer , Integer>  t_map = new TreeMap<>();
       	for( int i : arr ) {
       		t_map.put(i-k , t_map.getOrDefault(i-k, 0)+1);
       		t_map.put(i+k+1 , t_map.getOrDefault(i+k+1, 0) -1);
       	}
       	long ans = 0;
       	long  sum = 0;
       	for( int i : t_map.values()) {
       		sum+=i;
       		if(sum  > ans) {
       			ans = sum;
       		}
