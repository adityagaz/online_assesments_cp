...
public class amazon_oa_maximum_sum_subarray_from_two_non_overlapping_subarrays {
    ...
    static void solve( int n , int [] arr ) {
    	...
    	sum_left[0]=arr[0];
    	for ( int i = 1 ; i < n ; i++ ) {
    		sum_left[i] = Math.max(0 ,Math.max(sum_left[i-1] + arr[i] , arr[i]));
    	}

    	sum_right[n-1] =arr[n-1];
    	for (int i = n-2 ; i >= 0 ; i--) {
    		sum_right[i] = Math.max(Math.max(sum_right[i+1] + arr[i] , arr[i]) , 0);
    	}
    	int res = Integer.MIN_VALUE;
    	int [] ans = new int[n];
    	ans[0] = Math.max( sum_right[0]  , arr[0] +  sum_right[1]);
    	ans[n-1] = Math.max(sum_left[n-1] , arr[n-1] +  sum_left[n-2]);
    	for( int i = 1;  i < n-1 ; i++) {
    		res = Math.max(sum_left[i] + sum_right[i+1] , sum_right[i] + sum_left[i-1]);
    	}
    	printl(res);
    }
