...
public class Apple_OA {
    ...
    static void solve( ) {
        ...
        String opt1= concat(s1 , s2);
        String opt2= concat(s2 , s1);
    ...
    static String concat(String s1 , String s2 ) {
    	...
    	//easy apple OA question
         char toFind = s1.charAt(s1.length()-1);
         String res = "";
         List<Integer> indices = new ArrayList<>();
         for( int i = 0 ; i < s2.length() ; i++) {
             if(s2.charAt(i)==toFind)indices.add(i);
         }
         int mx = -1 , mx_idx = -1;
         for( int idx : indices) {
             ...
             if(i==-1)  {
                 mx_idx = Math.max(mx_idx, idx);
             }
         }
         if(mx_idx != -1) {
             res = s1 + s2.substring(mx_idx+1 , s2.length());
         }
         else {
             res = s1+ s2;
         }
