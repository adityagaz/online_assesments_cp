...
public class intuit_q4 implements Runnable {
    final static long mod = 1_000_000_007L;
    static PrintWriter out;


    public static void main(String[] args) {
        new Thread(null, new intuit_q4 (), "whatever", 1 << 30).start();
    }
    ...
    static void solve(FastReader sc ) throws Exception {
        ...
        while(t-- > 0) {
        	...
        	for( int c = 0; c < n ; c++) {
        		...
        		dp[0][c]=Math.min(mat[0][c], Integer.MAX_VALUE/2);
        		max = Math.max(max ,mat[0][c] - ( c>0 ? dp[0][c-1] : Integer.MAX_VALUE/2) );
        	}
        	for( int r = 0; r < n ; r++) {
        		dp[r][0]=Math.min(Integer.MAX_VALUE/2 , mat[r][0]);
        		max = Math.max(max ,mat[r][0] - ( r>0 ? dp[r-1][0] : Integer.MAX_VALUE/2) );
        	}
        	...
        	for( int i  = 1 ; i < n ; i++ ) {
        		for( int j= 1 ; j < m ; j++ ) {
        			...
        			int above = (i>0) ? dp[i-1][j] : Integer.MAX_VALUE/2;
        			max = Math.max(max , mat[i][j] - Math.min(left , above));
        			dp[i][j] = Math.min(mat[i][j] , Math.min(left , above));
        		}
        	}
