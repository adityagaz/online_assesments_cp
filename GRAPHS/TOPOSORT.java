...
public class TOPOSORT implements Runnable {
final static long mod = 1_000_000_007L;
static PrintWriter out;


public static void main(String[] args) {
    new Thread(null, new TOPOSORT (), "whatever", 1 << 30).start();
}
...
static void solve(FastReader sc ) throws Exception {
	...
	{
		int u = sc.nextInt();
		int v = sc.nextInt();g.get(u).add(v);
	}
	...
	System.out.println(topo);
		...
		System.out.println("Not possible cycle found");
...
static boolean dfs(int node )  {
	...
	for( int nei : g.get(node)) {
		...
		if(col[nei] == col[node]) return false;
	}
	col[node]=2;
	topo.add(node); // just add this one line in dfs
	return true;
}
