...
public class intuit_team_outing implements Runnable {
    final static long mod = 1_000_000_007L;
    static PrintWriter out;

    //https://www.desiqna.in/6149/intuit-oa-coding-questions-set-4-2022
    public static void main(String[] args) {
        new Thread(null, new intuit_team_outing (), "whatever", 1 << 30).start();
    }
    ...
    static long [][] mat, dp1 , dp2;
    static void solve(FastReader sc ) throws Exception {
    	// OH MY GOD !! THIS IS THE HARDEST PROBLEM I HAVE SEEN IN AN OA NP-HARD PROBLEM 
    	// INVOLVES BITMASK DP AND PARTITION DP
        // int t = sc.nextInt();
        int t = 1;
		...
		long[][] F = new long[FULL][N];

		for (int m = 0; m < FULL; m++) Arrays.fill(F[m], INF);

		// base: pick first employee u
		for (int u = 0; u < N; u++) {
		  int m = 1 << u;
		  F[m][u] = dist[0][u+1];               
		}
		...
		for (int i = 1; i <= N; i++) {
		  for (int j = i; j <= N; j++) {
		    ...
		    long best = INF;
		    for (int u = i; u <= j; u++) {
		      long cand = F[mask][u-1] + dist[u][N+1];   
		...
		for (int i = 1; i <= N; i++) dp2[i][i] = X + 2L*Y*dp1[i][i];
		...
		for (int len = 2; len <= N; len++) {
		  for (int i = 1; i + len - 1 <= N; i++) {
		    ...
		    long best = INF;
		    for (int k = i; k <= j; k++) {
		      long cand = (X + 2L*Y*dp1[i][k]) + dp2[k+1][j];
		      if (cand < best) best = cand;
		    }
		    dp2[i][j] = best;
		  }
