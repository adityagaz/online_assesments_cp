...
public class fox_and_two_dots implements Runnable {
    final static long mod = 1_000_000_007L;
    static PrintWriter out;


    public static void main(String[] args) {
        new Thread(null, new fox_and_two_dots (), "whatever", 1 << 30).start();
    }
    ...
    static boolean dfs(int i , int j , int pi , int pj , int n , int m) {
    	...
    	for( int [] d : dir) {
    		...
    		if(vis[ni][nj] == 1) {
    			...
    			if(dfs(ni , nj , i, j ,n , m)) return true;
    		}
    	}
    	return false;
    }
