import java.io.*;
import java.util.*;
// https://leetcode.com/discuss/post/3828380/intuit-online-assessment-on-campus-by-an-4jgi/
public class intuit_oa implements Runnable {
    ...
    static void solve(FastReader sc ) throws Exception {
        ...
        while(t-- > 0) {
        	...
        	Arrays.sort(arr, (x , y) -> 
        		Integer.compare((x[1]*y[0]) , (x[0]*y[1])));
        	int total =0;
        	int i = 0;
            int time = 0;
        	while(i<n) {
                int net = arr[i][2] - ((arr[i][1] + time) * arr[i][0]);
                if(net<0) {
                    i++;
                    continue;
                }
