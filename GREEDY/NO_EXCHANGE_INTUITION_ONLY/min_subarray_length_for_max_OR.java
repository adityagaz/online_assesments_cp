...
public class min_subarray_length_for_max_OR {
    ...
    static void solve( int  [] arr) {
    	...
    	int [] ans = new  int[n];
    	int [] latest_seen = new int[32];
    	Arrays.fill(latest_seen , -1);
    	for( int i  = n-1 ; i >= 0 ; i--) {
    		int max_req = 1;
    		for( int j = 0 ;j <= 31 ; j++ ) {
    			if((arr[i]&(1<<j)) != 0){
    				//here i made a mistake 
    				//(num & (1 << j) can be a large number not necessary 1 )
    				//so if we want to check that the but is set or not we can just check !=0 and not == 1 
    				//reason is our mask can go from 1 , 10 , 100 , 1000 like that...
    				latest_seen[j] = i;
    			}
    			else {
    				//means we have to search for that
    				if(latest_seen[j]==-1) {continue;}
    				else {
    					max_req = Math.max(max_req , latest_seen[j]-i+1);
    				}
    			}
    		}
