    ...
    static void solve(FastReader sc ) throws Exception {
        ...
        while(t-- > 0) {
        	...
        	Arrays.fill(dp2 , (int)-1e9);
           	...
           	/*	
           		dp1[i] -- means best answer till index i if we choose from arr1
           		dp2[i] -- means best answer till index i if we choose from arr2

           		for i = 1 .... N:

           			if(arr1[i] == arr2[i]) :
           				// we have option to take from both 
           				dp1[i] = arr1[i] + Math.max(dp1[i-1] , dp2[i-1]);
           				dp2[i] = arr2[i] + Math.max(dp1[i-1] , dp2[i-1]);
       				else :
       					take the own and continue
       					dp1[i] = arr1[i] + dp1[i-1];
       					dp2[i] = arr2[i] + dp2[i-1];
           	*/
        }
