...
public class intuit_oa implements Runnable {
    ...
    static void solve(FastReader sc ) throws Exception {
        ...
        while(t-- > 0) {
        	...
        	//dp[i] -- best answer to i such that all the points including the decimal ones are watered
        	// 
        	...
        	for(int i=0;i<n;i++){
        		...
        		while(j>=0){
        			int left= j-power[j];
        			int right = j+ power[j];
        			if(power[i]!=0){ // if all powers are zero then how can we 
        				// make the land between the points wet 1 ... 1.01. 1.02...1.03...1.99 2
        				// to water all points we not only need integer but decimal points too!!
        				if(right>=i ) {
        				ans = Math.min(ans , cost[j] + (left-1>=0 ? dp[left-1] : 0));
        			}
        			}
        			j--;
        		}
