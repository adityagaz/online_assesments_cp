...
public class no_palindromic_subsequences implements Runnable {
    final static long mod = 1_000_000_007L;
    static PrintWriter out;


    public static void main(String[] args) {
        new Thread(null, new no_palindromic_subsequences (), "whatever", 1 << 30).start();
    }
    ...
    static void solve(FastReader sc ) throws Exception {
        ...
        while(t-- > 0) {	
        	...
        	int no_palindromic_subsequences =0;
        	for( int i  = 0; i < n ; i++ ) {
        		dp[i][i] = 1; // all 1 length are palindromes 
        	}

        	for( int i = 0 ; i < n-1 ; i++ ) {
        		if(str.charAt(i) == str.charAt(i+1) ) {
        			dp[i][i+1] = 3; // if same then three, two individual and one combined 1+1+1 
        		}
        		else {
        			dp[i][i+1] =2; // only two individuals
        		}
        	}
        	...
        	for( int len = 3; len <=n; len++) {
        		...
        		for(int i  = 0; i<=n-len ; i++) {
        			...
        			if(str.charAt(i) == str.charAt(j)) {
        				...
        				//        contribution of i     contri of j alone       remove the overcounting part
        			}
        		}
        	}

           System.out.println(dp[0][n-1]); // dp[i][j] stores the no of palindromic subsequences from i...j
        }
